...
def test_init_main_executes_generator_with_built_spec(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "doc_gen.core.initialize.main.ScaffoldGenerator",
        FakeGenerator,
    )
    ...
    assert captured["templates"][0].target_path == Path(".config/doc_gen/config.toml")
    assert ".config/doc_gen" in captured["dirs"]
