...
def _write_distributions(
    ...
    wheel = dist_dir / f"doc_gen-{version}-py3-none-any.whl"
    with zipfile.ZipFile(wheel, mode="w") as archive:
        archive.writestr(f"doc_gen-{version}.dist-info/METADATA", metadata)
    sdist = dist_dir / f"doc-gen-{version}.tar.gz"
    with tarfile.open(sdist, mode="w:gz") as archive:
        info = tarfile.TarInfo(f"doc-gen-{version}/PKG-INFO")
        info.size = len(metadata)
        archive.addfile(info, io.BytesIO(metadata))
        egg_info = tarfile.TarInfo(f"doc-gen-{version}/doc_gen.egg-info/PKG-INFO")
...
def test_verify_distributions_checks_filenames_and_metadata(tmp_path: Path) -> None:
    ...
    assert verify_distributions("v1.2.3-rc.1", "doc-gen", tmp_path) == "1.2.3rc1"
...
def test_verify_distributions_rejects_noncanonical_metadata(tmp_path: Path) -> None:
    ...
    with pytest.raises(PackageVersionError, match="expected canonical PEP 440"):
        verify_distributions("v1.2.3-rc.1", "doc-gen", tmp_path)
