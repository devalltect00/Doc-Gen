...
"""Structural regression tests for Doc Gen developer workflows."""
...
def test_modular_make_workflows_are_installed() -> None:
    ...
    assert "l-generate-smart" in local_commands
    assert "l-print-smart" in local_commands
    assert "l-analyze-smart" in local_commands
...
def test_dockerfile_validates_runtime_and_accepts_an_explicit_version() -> None:
    ...
    assert "ARG DOC_GEN_BUILD_VERSION" in dockerfile
...
def test_published_images_receive_the_scm_package_version() -> None:
    ...
    assert "python -m doc_gen.core.build.version" in development
    assert (
        "DOC_GEN_BUILD_VERSION=${{ steps.package-version.outputs.value }}"
        in development
    )
    assert "python -m doc_gen.core.build.version" in production
...
def test_ignore_files_cover_local_test_workspaces() -> None:
    ...
    assert ".config/doc_gen/config.toml" in _read(".dockerignore")
